...
exports.default = function (state, actions) {
	...
	var match;
	for (var rule in Config.modules.HorairesDePrieres.rules) {	 
		match = (0, _helpers.syntax)(state.sentence, Config.modules.HorairesDePrieres.rules[rule]); 	
		if (match) break;
	}
	
	if (match) { // vérifié !
		if (state.debug) info('Intent HorairesDePrieres, Syntax: true');
		state.isIntent = true;
